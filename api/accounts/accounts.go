...
func (a *Accounts) getCode(addr luckyshare.Address, stateRoot luckyshare.Bytes32) ([]byte, error) {
	code, err := a.stater.
		NewState(stateRoot).
		GetCode(addr)
	if err != nil {
...
func (a *Accounts) handleGetCode(w http.ResponseWriter, req *http.Request) error {
	...
	addr, err := luckyshare.ParseAddress(hexAddr)
	if err != nil {
...
func (a *Accounts) getAccount(addr luckyshare.Address, header *block.Header) (*Account, error) {
...
func (a *Accounts) getStorage(addr luckyshare.Address, key luckyshare.Bytes32, stateRoot luckyshare.Bytes32) (luckyshare.Bytes32, error) {
...
func (a *Accounts) handleGetAccount(w http.ResponseWriter, req *http.Request) error {
	addr, err := luckyshare.ParseAddress(mux.Vars(req)["address"])
	if err != nil {
		return utils.BadRequest(errors.WithMessage(err, "address"))
	}
	h, err := a.handleRevision(req.URL.Query().Get("revision"))
...
func (a *Accounts) handleGetStorage(w http.ResponseWriter, req *http.Request) error {
	...
	key, err := luckyshare.ParseBytes32(mux.Vars(req)["key"])
	if err != nil {
...
func (a *Accounts) handleCallContract(w http.ResponseWriter, req *http.Request) error {
	...
	var addr *luckyshare.Address
	if mux.Vars(req)["address"] != "" {
		address, err := luckyshare.ParseAddress(mux.Vars(req)["address"])
		if err != nil {
			return utils.BadRequest(errors.WithMessage(err, "address"))
		}
		addr = &address
...
func (a *Accounts) handleBatchCallData(batchCallData *BatchCallData) (txCtx *xenv.TransactionContext, gas uint64, clauses []*tx.Clause, err error) {
	...
	if batchCallData.Caller == nil {
		txCtx.Origin = luckyshare.Address{}
	} else {
		txCtx.Origin = *batchCallData.Caller
	}
	if batchCallData.GasPayer == nil {
		txCtx.GasPayer = luckyshare.Address{}
	} else {
...
func (a *Accounts) handleRevision(revision string) (*block.Header, error) {
	...
	if len(revision) == 66 || len(revision) == 64 {
		blockID, err := luckyshare.ParseBytes32(revision)
		if err != nil {
