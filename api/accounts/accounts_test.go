...
import (
	...
	ABI "github.com/miniBamboo/luckyshare/abi"
...
var addr = luckyshare.BytesToAddress([]byte("to"))
var value = big.NewInt(10000)
var storageKey = luckyshare.Bytes32{}
var storageValue = byte(1)

var contractAddr luckyshare.Address
...
func getStorage(t *testing.T) {
	...
	h, err := luckyshare.ParseBytes32(value["value"])
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, luckyshare.BytesToBytes32([]byte{storageValue}), h, "storage should be equal")
	assert.Equal(t, http.StatusOK, statusCode, "OK")
}
...
func initAccountServer(t *testing.T) {
	...
	contractAddr = luckyshare.CreateContractAddress(transaction.ID(), 1, 0)
	...
	accounts.New(repo, stater, math.MaxUint64, luckyshare.NoFork).Mount(router, "/accounts")
...
func packTx(repo *chain.Repository, stater *state.Stater, transaction *tx.Transaction, t *testing.T) {
	b := repo.BestBlock()
	packer := packer.New(repo, stater, genesis.DevAccounts()[0].Address, &genesis.DevAccounts()[0].Address, luckyshare.NoFork)
