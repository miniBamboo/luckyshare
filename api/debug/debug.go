...
import (
	...
	"github.com/miniBamboo/luckyshare/api/utils"
	...
	"github.com/miniBamboo/luckyshare/tracers"
	"github.com/miniBamboo/luckyshare/trie"
	"github.com/miniBamboo/luckyshare/vm"
	"github.com/pkg/errors"
)

var devNetGenesisID = luckyshare.MustParseBytes32("0x00000000973ceb7f343a58b08f0693d6701a5fd354ff73d7058af3fba222aea4")
...
func New(repo *chain.Repository, stater *state.Stater, forkConfig luckyshare.ForkConfig) *Debug {
	return &Debug{
		repo,
		stater,
		forkConfig,
...
func (d *Debug) handleTxEnv(ctx context.Context, blockID luckyshare.Bytes32, txIndex uint64, clauseIndex uint64) (*runtime.Runtime, *runtime.TransactionExecutor, error) {
...
func (d *Debug) traceTransaction(ctx context.Context, tracer vm.Tracer, blockID luckyshare.Bytes32, txIndex uint64, clauseIndex uint64) (interface{}, error) {
...
func (d *Debug) debugStorage(ctx context.Context, contractAddress luckyshare.Address, blockID luckyshare.Bytes32, txIndex uint64, clauseIndex uint64, keyStart []byte, maxResult int) (*StorageRangeResult, error) {
...
func storageRangeAt(t *muxdb.Trie, start []byte, maxResult int) (*StorageRangeResult, error) {
	...
	for i := 0; i < maxResult && it.Next(); i++ {
		...
		v := luckyshare.BytesToBytes32(content)
		e := StorageEntry{Value: &v}
		if preimage := t.GetKeyPreimage(luckyshare.BytesToBytes32(it.Key)); len(preimage) > 0 {
			preimage := luckyshare.BytesToBytes32(preimage)
			e.Key = &preimage
		}
		result.Storage[luckyshare.BytesToBytes32(it.Key).String()] = e
	}
	if it.Next() {
		next := luckyshare.BytesToBytes32(it.Key)
		result.NextKey = &next
	}
	return &result, nil
}
...
func (d *Debug) parseTarget(target string) (blockID luckyshare.Bytes32, txIndex uint64, clauseIndex uint64, err error) {
	parts := strings.Split(target, "/")
	if len(parts) != 3 {
		return luckyshare.Bytes32{}, 0, 0, utils.BadRequest(errors.New("target:" + target + " unsupported"))
	}
	blockID, err = luckyshare.ParseBytes32(parts[0])
	if err != nil {
		return luckyshare.Bytes32{}, 0, 0, utils.BadRequest(errors.WithMessage(err, "target[0]"))
	}
	if len(parts[1]) == 64 || len(parts[1]) == 66 {
		txID, err := luckyshare.ParseBytes32(parts[1])
		if err != nil {
			return luckyshare.Bytes32{}, 0, 0, utils.BadRequest(errors.WithMessage(err, "target[1]"))
		}

		txMeta, err := d.repo.NewChain(blockID).GetTransactionMeta(txID)
		if err != nil {
			if d.repo.IsNotFound(err) {
				return luckyshare.Bytes32{}, 0, 0, utils.Forbidden(errors.New("transaction not found"))
			}
			return luckyshare.Bytes32{}, 0, 0, err
		}
	...
	if err != nil {
		return luckyshare.Bytes32{}, 0, 0, utils.BadRequest(errors.WithMessage(err, "target[2]"))
	}
	return
}
