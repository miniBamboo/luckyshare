...
func (s *Subscriptions) parsePosition(posStr string) (luckyshare.Bytes32, error) {
	bestID := s.repo.BestBlock().Header().ID()
	if posStr == "" {
		return bestID, nil
	}
	pos, err := luckyshare.ParseBytes32(posStr)
	if err != nil {
		return luckyshare.Bytes32{}, utils.BadRequest(errors.WithMessage(err, "pos"))
	}
	if block.Number(bestID)-block.Number(pos) > s.backtraceLimit {
		return luckyshare.Bytes32{}, utils.Forbidden(errors.New("pos: backtrace limit exceeded"))
	}
	return pos, nil
}

func parseTopic(t string) (*luckyshare.Bytes32, error) {
	if t == "" {
		return nil, nil
	}
	topic, err := luckyshare.ParseBytes32(t)
	if err != nil {
		return nil, err
	}
	return &topic, nil
...
func parseAddress(addr string) (*luckyshare.Address, error) {
	if addr == "" {
		return nil, nil
	}
	address, err := luckyshare.ParseAddress(addr)
	if err != nil {
		return nil, err
	}
	return &address, nil
