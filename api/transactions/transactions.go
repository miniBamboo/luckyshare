...
func (t *Transactions) getRawTransaction(txID luckyshare.Bytes32, head luckyshare.Bytes32, allowPending bool) (*rawTransaction, error) {
...
func (t *Transactions) getTransactionByID(txID luckyshare.Bytes32, head luckyshare.Bytes32, allowPending bool) (*Transaction, error) {
	tx, meta, err := t.repo.NewChain(head).GetTransaction(txID)
	if err != nil {
		if t.repo.IsNotFound(err) {
			if allowPending {
...
func (t *Transactions) getTransactionReceiptByID(txID luckyshare.Bytes32, head luckyshare.Bytes32) (*Receipt, error) {
...
func (t *Transactions) handleGetTransactionByID(w http.ResponseWriter, req *http.Request) error {
	id := mux.Vars(req)["id"]
	txID, err := luckyshare.ParseBytes32(id)
	if err != nil {
...
func (t *Transactions) parseHead(head string) (luckyshare.Bytes32, error) {
	if head == "" {
		return t.repo.BestBlock().Header().ID(), nil
	}
	h, err := luckyshare.ParseBytes32(head)
	if err != nil {
