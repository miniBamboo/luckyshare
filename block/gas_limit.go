...
func (gl GasLimit) IsValid(parentGasLimit uint64) bool {
	gasLimit := uint64(gl)
	if gasLimit < luckyshare.MinGasLimit {
		return false
	}
	var diff uint64
	...
	return diff <= parentGasLimit/luckyshare.GasLimitBoundDivisor
}
...
func (gl GasLimit) Qualify(parentGasLimit uint64) uint64 {
	gasLimit := uint64(gl)
	maxDiff := parentGasLimit / luckyshare.GasLimitBoundDivisor
	if gasLimit > parentGasLimit {
...
func (gl GasLimit) Adjust(delta int64) uint64 {
	gasLimit := uint64(gl)
	maxDiff := gasLimit / luckyshare.GasLimitBoundDivisor
	...
	// reduce
	diff := min64(uint64(-delta), maxDiff)
	if luckyshare.MinGasLimit+diff > gasLimit {
		// reach floor
		return luckyshare.MinGasLimit
	}
	return gasLimit - diff
}
