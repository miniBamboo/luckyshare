...
var (
	headKey = luckyshare.Blake2b([]byte("head"))
	tailKey = luckyshare.Blake2b([]byte("tail"))
)
...
func (a *Authority) getEntry(nodeMaster luckyshare.Address) (*entry, error) {
	var entry entry
	if err := a.state.DecodeStorage(a.addr, luckyshare.BytesToBytes32(nodeMaster[:]), func(raw []byte) error {
		if len(raw) == 0 {
			return nil
		}
...
func (a *Authority) setEntry(nodeMaster luckyshare.Address, entry *entry) error {
	return a.state.EncodeStorage(a.addr, luckyshare.BytesToBytes32(nodeMaster[:]), func() ([]byte, error) {
		if entry.IsEmpty() {
			return nil, nil
		}
...
func (a *Authority) getAddressPtr(key luckyshare.Bytes32) (addr *luckyshare.Address, err error) {
...
func (a *Authority) setAddressPtr(key luckyshare.Bytes32, addr *luckyshare.Address) error {
