...
func TestParamsNative(t *testing.T) {
	...
	b0 := buildGenesis(db, func(state *state.State) error {
		...
		builtin.Params.Native(state).Set(luckyshare.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		return nil
	})
...
func TestAuthorityNative(t *testing.T) {
	...
	b0 := buildGenesis(db, func(state *state.State) error {
		...
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(luckyshare.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		builtin.Params.Native(state).Set(luckyshare.KeyProposerEndorsement, luckyshare.InitialProposerEndorsement)
		return nil
	})
	repo, _ := chain.NewRepository(db, b0)
	st := state.New(db, b0.Header().StateRoot())
...
func TestPrototypeNative(t *testing.T) {
	...
	expected, err := st.GetStorage(builtin.Prototype.Address, luckyshare.Blake2b(contract.Bytes(), []byte("credit-plan")))
	assert.Nil(t, err)
	test.Case("storageFor", builtin.Prototype.Address, luckyshare.Blake2b(contract.Bytes(), []byte("credit-plan"))).
