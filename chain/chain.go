...
type Chain struct {
	repo     *Repository
	headID   luckyshare.Bytes32
	lazyInit func() (*muxdb.Trie, error)
}

func newChain(repo *Repository, headID luckyshare.Bytes32) *Chain {
	var (
...
func (c *Chain) GetBlockID(num uint32) (luckyshare.Bytes32, error) {
	...
	if len(data) == 0 {
		return luckyshare.Bytes32{}, errNotFound
	}
...
func (r *Repository) indexBlock(parentIndexRoot luckyshare.Bytes32, block *block.Block, receipts tx.Receipts) (luckyshare.Bytes32, error) {
	txs := block.Transactions()
	if len(txs) != len(receipts) {
		return luckyshare.Bytes32{}, errors.New("txs count != receipts count")
	}
