...
func NewRepository(db *muxdb.MuxDB, genesis *block.Block) (*Repository, error) {
	...
	if val, err := repo.props.Get(bestBlockIDKey); err != nil {
		...
		indexRoot, err := repo.indexBlock(luckyshare.Bytes32{}, genesis, nil)
		if err != nil {
			return nil, err
		}
		...
		bestID := luckyshare.BytesToBytes32(val)
...
func (r *Repository) saveBlock(block *block.Block, receipts tx.Receipts, indexRoot luckyshare.Bytes32) error {
	return r.data.Batch(func(putter kv.PutFlusher) error {
		var (
			header  = block.Header()
			id      = header.ID()
			txs     = block.Transactions()
			summary = BlockSummary{header, indexRoot, []luckyshare.Bytes32{}, uint64(block.Size())}
		)
