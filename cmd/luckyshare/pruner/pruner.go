...
func (p *Pruner) loop() error {
	...
	for {
		...
		case stepInitiate:
			...
			if status.N2 < luckyshare.MaxStateHistory {
				status.N2 = luckyshare.MaxStateHistory
			}
		...
		case stepDropStale:
			if err := waitUntil(status.N2 + luckyshare.MaxStateHistory + 128); err != nil {
				return err
			}
...
func (p *Pruner) archiveIndexTrie(pruner *muxdb.TriePruner, n1, n2 uint32) (nodeCount, entryCount int, err error) {
	var (
		...
		id1, id2, root1, root2 luckyshare.Bytes32
...
func (p *Pruner) archiveAccountTrie(pruner *muxdb.TriePruner, n1, n2 uint32) (nodeCount, entryCount, storageNodeCount, storageEntryCount int, err error) {
	...
	nodeCount, entryCount, err = pruner.ArchiveNodes(p.ctx, state.AccountTrieName, root1, root2, func(key, blob1, blob2 []byte) error {
		var sRoot1, sRoot2 luckyshare.Bytes32
		if len(blob1) > 0 {
			var acc state.Account
			if err := rlp.DecodeBytes(blob1, &acc); err != nil {
				return err
			}
			sRoot1 = luckyshare.BytesToBytes32(acc.StorageRoot)
		}
		if len(blob2) > 0 {
			var acc state.Account
			...
			sRoot2 = luckyshare.BytesToBytes32(acc.StorageRoot)
		}
		if sRoot1 != sRoot2 {
			n, e, err := pruner.ArchiveNodes(p.ctx, state.StorageTrieName(luckyshare.BytesToBytes32(key)), sRoot1, sRoot2, nil)
			if err != nil {
				return err
			}
			storageNodeCount += n
