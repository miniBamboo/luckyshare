...
func verifyLogDB(ctx context.Context, endBlockNum uint32, repo *chain.Repository, logDB *logdb.LogDB) error {
	...
	var (
		...
		logLimit    = uint32(0)
		splitEvLogs = func(id luckyshare.Bytes32) (logs []*logdb.Event) {
			if len(evLogs) == 0 {
				return
			}
		...
		splitTrLogs = func(id luckyshare.Bytes32) (logs []*logdb.Transfer) {
			if len(trLogs) == 0 {
				return
			}
...
func verifyLogDBPerBlock(
	...
	convertTopics := func(topics []luckyshare.Bytes32) (r [5]*luckyshare.Bytes32) {
