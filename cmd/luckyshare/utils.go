...
import (
	...
	"github.com/miniBamboo/luckyshare/p2psrv"
...
func defaultConfigDir() string {
	if home := homeDir(); home != "" {
		return filepath.Join(home, ".link.luckyshared.luckyshare")
	}
	return ""
}
...
func defaultDataDir() string {
	...
	if home := homeDir(); home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "Application Support", "org.vechain.luckyshare")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "link.luckyshared.luckyshare")
		} else {
			return filepath.Join(home, ".link.luckyshared.luckyshare")
		}
	}
...
func handleXGenesisID(h http.Handler, genesisID luckyshare.Bytes32) http.Handler {
...
func selectGenesis(ctx *cli.Context) (*genesis.Genesis, luckyshare.ForkConfig, error) {
	network := ctx.String(networkFlag.Name)
	if network == "" {
		_ = cli.ShowAppHelp(ctx)
		return nil, luckyshare.ForkConfig{}, errors.New("network flag not specified")
	}

	switch network {
	case "test":
		gene := genesis.NewTestnet()
		return gene, luckyshare.GetForkConfig(gene.ID()), nil
	case "main":
	...
	default:
		...
		if err != nil {
			return nil, luckyshare.ForkConfig{}, errors.Wrap(err, "open genesis file")
		}
		defer file.Close()
		...
		var forkConfig = luckyshare.NoFork
		var gen genesis.CustomGenesis
		gen.ForkConfig = &forkConfig

		if err := decoder.Decode(&gen); err != nil {
			return nil, luckyshare.ForkConfig{}, errors.Wrap(err, "decode genesis file")
		}

		customGen, err := genesis.NewCustomNet(&gen)
		if err != nil {
			return nil, luckyshare.ForkConfig{}, errors.Wrap(err, "build genesis")
		}
...
func beneficiary(ctx *cli.Context) (*luckyshare.Address, error) {
	value := ctx.String(beneficiaryFlag.Name)
	if value == "" {
		return nil, nil
	}
	addr, err := luckyshare.ParseAddress(value)
	if err != nil {
...
func newP2PComm(ctx *cli.Context, repo *chain.Repository, txPool *txpool.TxPool, instanceDir string) (*p2pComm, error) {
	...
	opts := &p2psrv.Options{
		Name:           common.MakeName("luckyshare", fullVersion()),
		PrivateKey:     key,
...
func startAPIServer(ctx *cli.Context, handler http.Handler, genesisID luckyshare.Bytes32) (string, func(), error) {
...
└────────────────────────────────────────────┴────────────────────────────────────────────────────────────────────┘`
	...
	for _, a := range genesis.DevAccounts() {
		info += fmt.Sprintf(tableContent,
			a.Address,
			luckyshare.BytesToBytes32(crypto.FromECDSA(a.PrivateKey)),
		)
	}
	info += tableEnd + "\r\n"
