...
import (
	...
	"github.com/miniBamboo/luckyshare/p2psrv/rpc"
)
...
func (p *Peer) Head() (id luckyshare.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
...
func (p *Peer) UpdateHead(id luckyshare.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
...
func (p *Peer) MarkTransaction(hash luckyshare.Bytes32) {
	// that's 1~5 block intervals
	expiration := int64(time.Second * time.Duration(luckyshare.BlockInterval*uint64(rand.Intn(5)+1)))
...
func (p *Peer) MarkBlock(id luckyshare.Bytes32) {
	p.knownBlocks.Add(id, struct{}{})
}

// IsTransactionKnown returns if the transaction is known.
func (p *Peer) IsTransactionKnown(hash luckyshare.Bytes32) bool {
...
func (p *Peer) IsBlockKnown(id luckyshare.Bytes32) bool {
