...
func txBuilder(tag byte) *tx.Builder {
	address := luckyshare.BytesToAddress([]byte("addr"))
...
func newTestConsensus(t *testing.T) *testConsensus {
	...
	p := packer.New(repo, stater, proposer.Address, &proposer.Address, luckyshare.NoFork)
...
func (tc *testConsensus) TestValidateBlockHeader() {
	...
	triggers["triggerErrFutureBlock"] = func() {
		build := tc.originalBuilder()
		blk := tc.sign(build.Timestamp(tc.time + luckyshare.BlockInterval*2).Build())
		err := tc.consent(blk)
...
func (tc *testConsensus) TestValidateBlockBody() {
	...
	triggers["triggerTxOriginBlocked"] = func() {
		luckyshare.MockBlocklist([]string{genesis.DevAccounts()[9].Address.String()})
	...
	triggers["triggerErrSignerInvalid"] = func() {
		...
		expect := consensusError(
			fmt.Sprintf(
				...
				luckyshare.Address(crypto.PubkeyToAddress(pk.PublicKey)),
			),
		)
		tc.assert.Equal(err, expect)
	}
	triggers["triggerErrTimestampUnscheduled"] = func() {
		...
		expect := consensusError(
			fmt.Sprintf(
				...
				blk.Header().Timestamp(),
				luckyshare.Address(crypto.PubkeyToAddress(genesis.DevAccounts()[1].PrivateKey.PublicKey)),
			),
		)
		tc.assert.Equal(err, expect)
	}
