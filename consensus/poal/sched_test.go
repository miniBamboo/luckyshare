...
func TestSchedule(t *testing.T) {
	...
	_, err := poal.NewScheduler(luckyshare.BytesToAddress([]byte("px")), proposers, 1, parentTime)
	assert.NotNil(t, err)

	sched, _ := poal.NewScheduler(p1, proposers, 1, parentTime)
...
func TestIsTheTime(t *testing.T) {
	sched, _ := poal.NewScheduler(p2, proposers, 1, parentTime)
