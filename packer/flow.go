...
type Flow struct {
	...
	processedTxs map[luckyshare.Bytes32]bool // txID -> reverted
	gasUsed      uint64
	txs          tx.Transactions
...
func newFlow(
	...
	return &Flow{
		...
		runtime:      runtime,
		processedTxs: make(map[luckyshare.Bytes32]bool),
		features:     features,
	}
}
...
func (f *Flow) findTx(txID luckyshare.Bytes32) (found bool, reverted bool, err error) {
...
func (f *Flow) Adopt(tx *tx.Transaction) error {
	origin, _ := tx.Origin()
	if f.runtime.Context().Number >= f.packer.forkConfig.BLOCKLIST && luckyshare.IsOriginBlocked(origin) {
	...
	case f.gasUsed+tx.Gas() > f.runtime.Context().GasLimit:
		...
		if f.gasUsed+luckyshare.TxGas+luckyshare.ClauseGas <= f.runtime.Context().GasLimit {
...
func (f *Flow) Pack(privateKey *ecdsa.PrivateKey) (*block.Block, *state.Stage, tx.Receipts, error) {
	if f.packer.nodeMaster != luckyshare.Address(crypto.PubkeyToAddress(privateKey.PublicKey)) {
