...
func (ti *txIterator) OnProcessed(txID luckyshare.Bytes32, err error) {
}
...
func TestP(t *testing.T) {
	...
	for {
		best := repo.BestBlock()
		p := packer.New(repo, stater, a1.Address, &a1.Address, luckyshare.NoFork)
		...
		fmt.Println(consensus.New(repo, stater, luckyshare.NoFork).Process(blk, uint64(time.Now().Unix()*2)))
...
func TestForkVIP191(t *testing.T) {
	...
	best := repo.BestBlock()
	p := packer.New(repo, stater, a1.Address, &a1.Address, luckyshare.ForkConfig{
		VIP191: 1,
	})
	flow, err := p.Schedule(best.Header(), uint64(time.Now().Unix()))
	if err != nil {
	...
	_, _, err = consensus.New(repo, stater, luckyshare.ForkConfig{}).Process(blk, uint64(time.Now().Unix()*2))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBlocklist(t *testing.T) {
	...
	luckyshare.MockBlocklist([]string{a0.Address.String()})
