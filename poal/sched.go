...
func (s *Scheduler) Schedule(nowTime uint64) (newBlockTime uint64) {
	const T = luckyshare.BlockInterval
...
func (s *Scheduler) IsTheTime(newBlockTime uint64) bool {
	...
	if (newBlockTime-s.parentBlockTime)%luckyshare.BlockInterval != 0 {
		// invalid block time
		return false
	}
...
func (s *Scheduler) Updates(newBlockTime uint64) (updates []Proposer, score uint64) {
	...
	toDeactivate := make(map[luckyshare.Address]Proposer)

	t := newBlockTime - luckyshare.BlockInterval
	for i := uint64(0); i < luckyshare.MaxBlockProposers && t > s.parentBlockTime; i++ {
		p := s.whoseTurn(t)
		if p.Address != s.proposer.Address {
			toDeactivate[p.Address] = p
		}
		t -= luckyshare.BlockInterval
	}
...
func dprp(blockNumber uint32, time uint64) uint64 {
	...
	return binary.BigEndian.Uint64(luckyshare.Blake2b(b4[:], b8[:]).Bytes())
}
