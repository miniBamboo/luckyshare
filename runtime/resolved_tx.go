...
func (r *ResolvedTransaction) CommonTo() *luckyshare.Address {
	if len(r.Clauses) == 0 {
		return nil
	}
...
func (r *ResolvedTransaction) BuyGas(state *state.State, blockTime uint64) (
	...
	if r.Delegator != nil {
		...
		return nil, nil, luckyshare.Address{}, nil, errors.New("insufficient energy")
	}
	...
	if commonTo != nil {
		...
		if credit.Cmp(prepaid) >= 0 {
			...
			var sponsor luckyshare.Address
...
func (r *ResolvedTransaction) ToContext(
	...
	blockNumber uint32,
	getBlockID func(uint32) (luckyshare.Bytes32, error),
