...
func (tr *testResolvedTransaction) TestCommonTo() {
	...
	address := luckyshare.BytesToAddress([]byte("addr1"))
...
func (tr *testResolvedTransaction) TestBuyGas() {
	...
	targetTime := tr.repo.BestBlock().Header().Timestamp() + luckyshare.BlockInterval

	buyGas := func(tx *tx.Transaction) luckyshare.Address {
		resolve, err := runtime.ResolveTransaction(tx)
		if err != nil {
			tr.t.Fatal(err)
		}
