...
type Output struct {
	...
	ContractAddress *luckyshare.Address // if create a new contract, or is nil.
}
...
func (rt *Runtime) newEVM(stateDB *statedb.StateDB, clauseIndex uint32, txCtx *xenv.TransactionContext) *vm.EVM {
	...
	return vm.NewEVM(vm.Context{
		...
		Transfer: func(_ vm.StateDB, sender, recipient common.Address, amount *big.Int) {
			...
			senderEnergy, err := rt.state.GetEnergy(luckyshare.Address(sender), rt.ctx.Time)
			if err != nil {
				panic(err)
			}
			recipientEnergy, err := rt.state.GetEnergy(luckyshare.Address(recipient), rt.ctx.Time)
			if err != nil {
				panic(err)
			}

			if err := rt.state.SetEnergy(luckyshare.Address(sender), senderEnergy, rt.ctx.Time); err != nil {
				panic(err)
			}
			if err := rt.state.SetEnergy(luckyshare.Address(recipient), recipientEnergy, rt.ctx.Time); err != nil {
				panic(err)
			}
			...
			stateDB.AddTransfer(&tx.Transfer{
				Sender:    luckyshare.Address(sender),
				Recipient: luckyshare.Address(recipient),
		...
		NewContractAddress: func(_ *vm.EVM, counter uint32) common.Address {
			return common.Address(luckyshare.CreateContractAddress(txCtx.ID, clauseIndex, counter))
		},
		...
		OnCreateContract: func(_ *vm.EVM, contractAddr, caller common.Address) {
			...
			if err := rt.state.SetMaster(luckyshare.Address(contractAddr), luckyshare.Address(caller)); err != nil {
				panic(err)
			}
		...
		OnSuicideContract: func(_ *vm.EVM, contractAddr, tokenReceiver common.Address) {
			...
			amount, err := rt.state.GetEnergy(luckyshare.Address(contractAddr), rt.ctx.Time)
			if err != nil {
				panic(err)
			}
			if amount.Sign() != 0 {
				...
				receiverEnergy, err := rt.state.GetEnergy(luckyshare.Address(tokenReceiver), rt.ctx.Time)
				if err != nil {
					panic(err)
				}
				if err := rt.state.SetEnergy(
					luckyshare.Address(tokenReceiver),
			...
			if amount := stateDB.GetBalance(contractAddr); amount.Sign() != 0 {
				...
				stateDB.AddTransfer(&tx.Transfer{
					Sender:    luckyshare.Address(contractAddr),
					Recipient: luckyshare.Address(tokenReceiver),
					Amount:    amount,
				})
			}
		},
...
func (rt *Runtime) PrepareClause(
	...
	var (
		...
		leftOverGas   uint64
		vmErr         error
		contractAddr  *luckyshare.Address
	...
	exec = func() (output *Output, interrupted bool, err error) {
		...
		if clause.To() == nil {
			...
			contractAddr = (*luckyshare.Address)(&caddr)
		} else {
