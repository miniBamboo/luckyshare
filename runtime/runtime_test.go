...
func TestContractSuicide(t *testing.T) {
	...
	addr := luckyshare.BytesToAddress([]byte("acc01"))
	...
	exec, _ := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{Time: time}, luckyshare.NoFork).
...
func TestCall(t *testing.T) {
	...
	assert.Equal(t, luckyshare.Address(addr), genesis.DevAccounts()[0].Address)
...
func TestExecuteTransaction(t *testing.T) {
	...
	// addr1 := luckyshare.Address(crypto.PubkeyToAddress(key.PublicKey))
	// addr2 := luckyshare.BytesToAddress([]byte("acc2"))
	...
	// 	luckyshare.Address{}, 0, 0, 0, func(uint32) luckyshare.Bytes32 { return luckyshare.Bytes32{} })
