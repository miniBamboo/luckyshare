...
// 	s.state.ForEachStorage(luckyshare.Address(addr), func(k luckyshare.Bytes32, v []byte) bool {
...
func (s *StateDB) GetBalance(addr common.Address) *big.Int {
	bal, err := s.state.GetBalance(luckyshare.Address(addr))
	if err != nil {
		panic(err)
	}
	return bal
...
func (s *StateDB) SubBalance(addr common.Address, amount *big.Int) {
	...
	balance, err := s.state.GetBalance(luckyshare.Address(addr))
	if err != nil {
		panic(err)
	}
	if err := s.state.SetBalance(luckyshare.Address(addr), new(big.Int).Sub(balance, amount)); err != nil {
		panic(err)
	}
}
...
func (s *StateDB) AddBalance(addr common.Address, amount *big.Int) {
	...
	if err := s.state.SetBalance(luckyshare.Address(addr), new(big.Int).Add(balance, amount)); err != nil {
		panic(err)
	}
}
...
func (s *StateDB) GetCodeHash(addr common.Address) common.Hash {
	hash, err := s.state.GetCodeHash(luckyshare.Address(addr))
	if err != nil {
		panic(err)
	}
...
func (s *StateDB) GetCode(addr common.Address) []byte {
	code, err := s.state.GetCode(luckyshare.Address(addr))
	if err != nil {
		panic(err)
	}
	return code
...
func (s *StateDB) SetCode(addr common.Address, code []byte) {
	if err := s.state.SetCode(luckyshare.Address(addr), code); err != nil {
		panic(err)
	}
}
...
func (s *StateDB) Suicide(addr common.Address) bool {
	exist, err := s.state.Exists(luckyshare.Address(addr))
	if err != nil {
		panic(err)
	}
	if !exist {
	...
	s.state.Delete(luckyshare.Address(addr))
...
func (s *StateDB) GetState(addr common.Address, key common.Hash) common.Hash {
	val, err := s.state.GetStorage(luckyshare.Address(addr), luckyshare.Bytes32(key))
	if err != nil {
		panic(err)
	}
...
func (s *StateDB) SetState(addr common.Address, key, value common.Hash) {
	s.state.SetStorage(luckyshare.Address(addr), luckyshare.Bytes32(key), luckyshare.Bytes32(value))
}

// Exist stub.
func (s *StateDB) Exist(addr common.Address) bool {
	b, err := s.state.Exists(luckyshare.Address(addr))
	if err != nil {
		panic(err)
	}
	return b
...
func ethlogToEvent(ethlog *types.Log) *tx.Event {
	var topics []luckyshare.Bytes32
	if len(ethlog.Topics) > 0 {
		topics = make([]luckyshare.Bytes32, 0, len(ethlog.Topics))
		for _, t := range ethlog.Topics {
			topics = append(topics, luckyshare.Bytes32(t))
		}
	}
	return &tx.Event{
		luckyshare.Address(ethlog.Address),
		topics,
		ethlog.Data,
	}
}
