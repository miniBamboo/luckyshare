...
var (
	initialSupplyKey = luckyshare.Blake2b([]byte("initial-supply"))
	totalAddSubKey   = luckyshare.Blake2b([]byte("total-add-sub"))
)
...
func New(addr luckyshare.Address, state *state.State, blockTime uint64) *Energy {
...
func (e *Energy) Get(addr luckyshare.Address) (*big.Int, error) {
	return e.state.GetEnergy(addr, e.blockTime)
}

// Add add amount of energy to given address.
func (e *Energy) Add(addr luckyshare.Address, amount *big.Int) error {
	if amount.Sign() == 0 {
		return nil
	}
...
func (e *Energy) Sub(addr luckyshare.Address, amount *big.Int) (bool, error) {
	if amount.Sign() == 0 {
		return true, nil
	}
	eng, err := e.state.GetEnergy(addr, e.blockTime)
