...
func approverEvent(approver luckyshare.Address, action string) *tx.Event {
	ev, _ := sharer.Executor.ABI.EventByName("Approver")
	var b32 luckyshare.Bytes32
	copy(b32[:], action)
	data, _ := ev.Encode(b32)
	return &tx.Event{
		Address: sharer.Executor.Address,
...
func proposalEvent(id luckyshare.Bytes32, action string) *tx.Event {
	ev, _ := sharer.Executor.ABI.EventByName("Proposal")
	var b32 luckyshare.Bytes32
	copy(b32[:], action)
	data, _ := ev.Encode(b32)
	return &tx.Event{
...
func votingContractEvent(addr luckyshare.Address, action string) *tx.Event {
	ev, _ := sharer.Executor.ABI.EventByName("VotingContract")
	var b32 luckyshare.Bytes32
	copy(b32[:], action)
	data, _ := ev.Encode(b32)
	return &tx.Event{
...
func initExectorTest() *ctest {
	...
	b0 := buildGenesis(db, func(state *state.State) error {
		state.SetCode(sharer.Prototype.Address, sharer.Prototype.RuntimeBytecodes())
		state.SetCode(sharer.Executor.Address, sharer.Executor.RuntimeBytecodes())
		state.SetCode(sharer.Params.Address, sharer.Params.RuntimeBytecodes())
		sharer.Params.Native(state).Set(luckyshare.KeyExecutorAddress, new(big.Int).SetBytes(sharer.Executor.Address[:]))
		return nil
	})
	...
	return &ctest{
		rt:  rt,
		abi: sharer.Executor.ABI,
		to:  sharer.Executor.Address,
	}
}
...
func TestExecutorApprover(t *testing.T) {
	...
	for _, a := range approvers {
		...
		test.Case("addApprover", a, luckyshare.BytesToBytes32(a.Bytes())).
			Caller(sharer.Executor.Address).
			ShouldLog(approverEvent(a, "added")).
			Assert(t)
		assert.Equal(t, M(true, nil), M(sharer.Prototype.Native(test.rt.State()).Bind(test.to).IsUser(a)))
	}
	...
	for _, a := range approvers {
		...
		test.Case("revokeApprover", a).
			...
			Assert(t)
		assert.Equal(t, M(false, nil), M(sharer.Prototype.Native(test.rt.State()).Bind(test.to).IsUser(a)))
	}
	test.Case("approverCount").
...
func TestExecutorProposal(t *testing.T) {
	...
	target := sharer.Params.Address
	setParam, _ := sharer.Params.ABI.MethodByName("set")
	...
	assert.Equal(t, M(big.NewInt(12345), nil), M(sharer.Params.Native(test.rt.State()).Get(luckyshare.BytesToBytes32([]byte("paramKey")))))
}
