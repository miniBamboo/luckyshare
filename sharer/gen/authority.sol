...
contract Authority {
    ...
    function add(address _nodeMaster, address _endorsor, bytes32 _identity) public {
        require(_nodeMaster != 0, "sharer: invalid node master");
        require(_endorsor != 0, "sharer: invalid endorsor");
        require(_identity != 0, "sharer: invalid identity");
        require(msg.sender == executor(), "sharer: executor required");

        require(AuthorityNative(this).native_add(_nodeMaster, _endorsor, _identity), "sharer: already exists");

        emit Candidate(_nodeMaster, "added");
    }

    function revoke(address _nodeMaster) public {
        require(msg.sender == executor() || !AuthorityNative(this).native_isEndorsed(_nodeMaster), "sharer: requires executor, or node master out of endorsed");
        require(AuthorityNative(this).native_revoke(_nodeMaster), "sharer: not listed");
