...
contract Executor {
    ...
    function propose(address _target, bytes _data) public returns(bytes32) {
        require(_target != 0, "sharer: invalid target");
        require(approverCount > 0, "sharer: no approvers");
        require(approvers[msg.sender].inPower || votingContracts[msg.sender], "sharer: approver or voting contract required");

        bytes32 proposalID = keccak256(abi.encodePacked(uint64(now), msg.sender));
        require(proposals[proposalID].timeProposed == 0, "sharer: duplicated proposal id");
    ...
    function approve(bytes32 _proposalID) public {
        require(proposals[_proposalID].timeProposed > 0, "sharer: proposal not found");
        require(approvers[msg.sender].inPower, "sharer: approver required");
        require(now - proposals[_proposalID].timeProposed < 1 weeks, "sharer: proposal expired");
        require(!proposals[_proposalID].approvals[msg.sender], "sharer: proposal approved");
    ...
    function execute(bytes32 _proposalID) public {
        require(proposals[_proposalID].timeProposed > 0, "sharer: proposal not found");
        require(!proposals[_proposalID].executed, "sharer: proposal executed");
        require(now - proposals[_proposalID].timeProposed < 1 weeks, "sharer: proposal expired");
        require(proposals[_proposalID].approvalCount >= proposals[_proposalID].quorum, "sharer: quorum unsatisfied");

        proposals[_proposalID].executed = true; // set before call to prevent re-enter attack
        require(proposals[_proposalID].target.call(proposals[_proposalID].data), "sharer: proposal execution reverted");
    ...
    function addApprover(address _approver, bytes32 _identity) public {
        onlyThis();
        require(_approver != 0, "sharer: invalid approver");
        require(_identity != 0, "sharer: invalid identity");
        require(approvers[_approver].identity == 0, "sharer: approver exists");
        require(approverCount < 255, "sharer: too many approvers");
    ...
    function revokeApprover(address _approver) public {
        onlyThis();
        require(approvers[_approver].inPower, "sharer: unknown or revoked approver");
    ...
    function attachVotingContract(address _contract) public {
        onlyThis();
        require(_contract != 0, "sharer: invalid contract address");
        require(!votingContracts[_contract], "sharer: voting contract already attached");
    ...
    function detachVotingContract(address _contract) public {
        onlyThis();
        require(votingContracts[_contract], "sharer: voting contract not attached");
    ...
    function onlyThis() internal view {
        require(msg.sender == address(this), "sharer: executor required");
    }
