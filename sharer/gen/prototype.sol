...
contract Prototype {
    ...
    function addUser(address _self, address _user) public{
        require(_user != 0, "sharer: invalid user");
        onlySelfOrMaster(_self);
        require(PrototypeNative(this).native_addUser(_self, _user), "sharer: already added");
    }

    function removeUser(address _self, address _user) public{
        onlySelfOrMaster(_self);
        require(PrototypeNative(this).native_removeUser(_self, _user), "sharer: not a user");
    }

    function sponsor(address _self) public{
        require(PrototypeNative(this).native_sponsor(_self, msg.sender), "sharer: already sponsored");
    }

    function unsponsor(address _self) public {
        require(PrototypeNative(this).native_unsponsor(_self, msg.sender), "sharer: not sponsored");
    }
    ...
    function selectSponsor(address _self, address _sponsor) public {
        onlySelfOrMaster(_self);
        require(PrototypeNative(this).native_selectSponsor(_self, _sponsor), "sharer: not a sponsor");
    }
    ...
    function onlySelfOrMaster(address _self) internal view {
        require(_self == msg.sender || PrototypeNative(this).native_master(_self) == msg.sender, "sharer: self or master required");
    }
}
