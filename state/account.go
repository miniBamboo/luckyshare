...
func (a *Account) CalcEnergy(blockTime uint64) *big.Int {
	...
	x.Mul(x, a.Balance)
	x.Mul(x, luckyshare.EnergyGrowthRate)
	x.Div(x, bigE18)
...
func loadAccount(trie *muxdb.Trie, addr luckyshare.Address) (*Account, error) {
...
func saveAccount(trie *muxdb.Trie, addr luckyshare.Address, a *Account) error {
	if a.IsEmpty() {
...
func loadStorage(trie *muxdb.Trie, key luckyshare.Bytes32) (rlp.RawValue, error) {
	return trie.Get(key[:])
}

// saveStorage save value for given key.
// If the data is zero, the given key will be deleted.
func saveStorage(trie *muxdb.Trie, key luckyshare.Bytes32, data rlp.RawValue) error {
