...
type nodeIteratorState struct {
	hash    luckyshare.Bytes32 // Hash of the node being iterated (nil if not standalone)
	node    node               // Trie node being iterated
	enc     []byte
	parent  luckyshare.Bytes32 // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() luckyshare.Bytes32 {
	if len(it.stack) == 0 {
		return luckyshare.Bytes32{}
	}
...
func (it *nodeIterator) Parent() luckyshare.Bytes32 {
	if len(it.stack) == 0 {
		return luckyshare.Bytes32{}
	}
...
func (it *nodeIterator) peek(descend bool) (*nodeIteratorState, *int, []byte, error) {
	...
	for len(it.stack) > 0 {
		...
		if (ancestor == luckyshare.Bytes32{}) {
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.enc = enc
		st.hash = luckyshare.BytesToBytes32(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor luckyshare.Bytes32) (*nodeIteratorState, []byte, bool) {
		...
		for i := parent.index + 1; i < len(node.Children); i++ {
			...
			if child != nil {
				...
				state := &nodeIteratorState{
					hash:    luckyshare.BytesToBytes32(hash),
					node:    child,
					parent:  ancestor,
					index:   -1,
					pathlen: len(it.path),
...
func (it *differenceIterator) Hash() luckyshare.Bytes32 {
	return it.b.Hash()
}
...
func (it *differenceIterator) Parent() luckyshare.Bytes32 {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == luckyshare.Bytes32{}
...
func (it *unionIterator) Hash() luckyshare.Bytes32 {
...
func (it *unionIterator) Parent() luckyshare.Bytes32 {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == luckyshare.Bytes32{}) {
			it.count++
