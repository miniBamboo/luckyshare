...
func makeTestTrie() (ethdb.Database, *Trie, map[string][]byte) {
	...
	trie, _ := New(luckyshare.Bytes32{}, db)
...
func checkTrieContents(t *testing.T, db Database, root []byte, content map[string][]byte) {
	...
	trie, err := New(luckyshare.BytesToBytes32(root), db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, luckyshare.BytesToBytes32(root)); err != nil {
...
func checkTrieConsistency(db Database, root luckyshare.Bytes32) error {
...
func TestEmptyTrieSync(t *testing.T) {
	emptyA, _ := New(luckyshare.Bytes32{}, nil)
	emptyB, _ := New(emptyRoot, nil)

	for i, trie := range []*Trie{emptyA, emptyB} {
		db := ethdb.NewMemDatabase()
		if req := NewTrieSync(luckyshare.BytesToBytes32(trie.Root()), db, nil).Missing(1); len(req) != 0 {
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	sched := NewTrieSync(luckyshare.BytesToBytes32(srcTrie.Root()), dstDb, nil)

	queue := append([]luckyshare.Bytes32{}, sched.Missing(batch)...)
	for len(queue) > 0 {
...
func TestIterativeDelayedTrieSync(t *testing.T) {
	...
	queue := append([]luckyshare.Bytes32{}, sched.Missing(10000)...)
	for len(queue) > 0 {
...
func testIterativeRandomTrieSync(t *testing.T, batch int) {
	...
	queue := make(map[luckyshare.Bytes32]struct{})
...
func TestDuplicateAvoidanceTrieSync(t *testing.T) {
	...
	queue := append([]luckyshare.Bytes32{}, sched.Missing(0)...)
	requested := make(map[luckyshare.Bytes32]struct{})
...
func TestIncompleteTrieSync(t *testing.T) {
	...
	added := []luckyshare.Bytes32{}
	queue := append([]luckyshare.Bytes32{}, sched.Missing(1)...)
	for len(queue) > 0 {
