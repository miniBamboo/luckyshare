...
type body struct {
	...
	GasPriceCoef uint8
	Gas          uint64
	DependsOn    *luckyshare.Bytes32 `rlp:"nil"`
	Nonce        uint64
	Reserved     reserved
	Signature    []byte
}
...
func (t *Transaction) ID() (id luckyshare.Bytes32) {
...
func (t *Transaction) Hash() (hash luckyshare.Bytes32) {
...
func (t *Transaction) EvaluateWork(origin luckyshare.Address) func(nonce uint64) *big.Int {
	hw := luckyshare.NewBlake2b()
	...
	var hashWithoutNonce luckyshare.Bytes32
	hw.Sum(hashWithoutNonce[:0])

	return func(nonce uint64) *big.Int {
		var nonceBytes [8]byte
		binary.BigEndian.PutUint64(nonceBytes[:], nonce)
		hash := luckyshare.Blake2b(hashWithoutNonce[:], nonceBytes[:])
...
func (t *Transaction) SigningHash() (hash luckyshare.Bytes32) {
...
func (t *Transaction) DependsOn() *luckyshare.Bytes32 {
...
func (t *Transaction) Origin() (luckyshare.Address, error) {
	...
	origin := luckyshare.Address(crypto.PubkeyToAddress(*pub))
...
func (t *Transaction) DelegatorSigningHash(origin luckyshare.Address) (hash luckyshare.Bytes32) {
	hw := luckyshare.NewBlake2b()
	hw.Write(t.SigningHash().Bytes())
	hw.Write(origin.Bytes())
	hw.Sum(hash[:0])
...
func (t *Transaction) Delegator() (*luckyshare.Address, error) {
	if err := t.validateSignatureLength(); err != nil {
		return nil, err
	}
	...
	if cached := t.cache.delegator.Load(); cached != nil {
		addr := cached.(luckyshare.Address)
		return &addr, nil
	}
	...
	delegator := luckyshare.Address(crypto.PubkeyToAddress(*pub))
...
func (t *Transaction) ProvedWork(headBlockNum uint32, getBlockID func(uint32) (luckyshare.Bytes32, error)) (*big.Int, error) {
	ref := t.BlockRef()
	refNum := ref.Number()
	...
	if delay := headBlockNum - refNum; delay > luckyshare.MaxTxWorkDelay {
		return &big.Int{}, nil
	}
...
func IntrinsicGas(clauses ...*Clause) (uint64, error) {
	if len(clauses) == 0 {
		return luckyshare.TxGas + luckyshare.ClauseGas, nil
	}

	var total = luckyshare.TxGas
	var overflow bool
	for _, c := range clauses {
		...
		if c.IsCreatingContract() {
			// contract creation
			cgas = luckyshare.ClauseGasContractCreation
		} else {
			cgas = luckyshare.ClauseGas
		}
