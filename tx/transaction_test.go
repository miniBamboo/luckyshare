...
func TestTx(t *testing.T) {
	to, _ := luckyshare.ParseAddress("0x7567d83b7b8d80addcb281a71d54fc7b3364ffed")
	...
	assert.Equal(t, luckyshare.Bytes32{}, trx.ID())
...
func TestDelegatedTx(t *testing.T) {
	...
	hash := trx.DelegatorSigningHash(luckyshare.Address(o))
...
func TestIntrinsicGas(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, luckyshare.TxGas+luckyshare.ClauseGas, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(&luckyshare.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, luckyshare.TxGas+luckyshare.ClauseGas, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(nil))
	assert.Nil(t, err)
	assert.Equal(t, luckyshare.TxGas+luckyshare.ClauseGasContractCreation, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(&luckyshare.Address{}), tx.NewClause(&luckyshare.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, luckyshare.TxGas+luckyshare.ClauseGas*2, gas)
}

func BenchmarkTxMining(b *testing.B) {
	tx := new(tx.Builder).Build()
	signer := luckyshare.BytesToAddress([]byte("acc1"))
	maxWork := &big.Int{}
