...
func newTx(chainTag byte, clauses []*tx.Clause, gas uint64, blockRef tx.BlockRef, expiration uint32, dependsOn *luckyshare.Bytes32, features tx.Features, from genesis.DevAccount) *tx.Transaction {
...
func TestExecutable(t *testing.T) {
	...
	tests := []struct {
		...
		{newTx(0, nil, 21000, tx.BlockRef{0}, 100, &luckyshare.Bytes32{}, tx.Features(0), acc), false, ""},
	}
