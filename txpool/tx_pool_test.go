...
func TestAdd(t *testing.T) {
	...
	tests = []struct {
		...
		{newTx(pool.repo.ChainTag(), nil, 21000, tx.BlockRef{}, 100, &luckyshare.Bytes32{1}, Tx.Features(0), acc), "tx rejected: tx is not executable"},
		{newTx(pool.repo.ChainTag(), nil, 21000, tx.BlockRef{}, 100, &luckyshare.Bytes32{1}, Tx.Features(2), acc), "tx rejected: unsupported features"},
